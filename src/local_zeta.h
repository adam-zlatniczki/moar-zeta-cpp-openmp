...
/**
 * @brief Calculate the local zeta metrics in both directions from the sample.
 *
 * @param x The X sample points.
 * @param y The Y sample points.
 * @param n Number of sample points.
 * @param k Number of neighbors (excluding self) to find.
 * @param local_zeta_x Pointer where the local zetas of X should be stored.
 * @param local_zeta_y Pointer where the local zetas of X should be stored.
 */
void get_local_zeta(const double* x, const double* y, unsigned int n, unsigned int k, double* local_zeta_x, double* local_zeta_y);
